...
class BotMapCommand extends Command
{
     ...
     */
    protected function generateMermaidDiagram(): string
    {
        $lines = ["flowchart TD\n"];

        // Группируем по секциям
        $sections = $this->screens->groupBy(fn($s) => $this->getSection($s->key));

        foreach ($sections as $sectionName => $sectionScreens) {
            $lines[] = "    subgraph " . ucfirst($sectionName);
            
            foreach ($sectionScreens as $screen) {
                $nodeId = $this->toNodeId($screen->key);
        ...
        // Связи
        $lines[] = "    %% Связи между экранами";
        
        foreach ($this->screens as $screen) {
            $fromId = $this->toNodeId($screen->key);
            ...
            foreach ($screen->buttons as $button) {
                ...
                if ($isBroken) {
                    $toId = $this->toNodeId($button->next_screen_key) . "_broken[\"X {$button->next_screen_key}\"]";
                    $lines[] = "    {$fromId} -.->|\"{$buttonText}\"| {$toId}";
                } else {
                    $toId = $this->toNodeId($button->next_screen_key);
                    $lines[] = "    {$fromId} -->|\"{$buttonText}\"| {$toId}";
                }
            }
        }
        ...
        $lines[] = "\n    %% Стили";
        $colors = [
            'main' => 'fill:#dcfce7,stroke:#22c55e',
            'faq' => 'fill:#dbeafe,stroke:#3b82f6',
            'tariffs' => 'fill:#ffedd5,stroke:#f97316',
            'account' => 'fill:#f3e8ff,stroke:#a855f7',
            'support' => 'fill:#cffafe,stroke:#06b6d4',
            'troubleshoot' => 'fill:#fee2e2,stroke:#ef4444',
            'docs' => 'fill:#fef3c7,stroke:#f59e0b',
        ];
        ...
        foreach ($this->screens as $screen) {
            ...
            if (isset($colors[$section])) {
                $nodeId = $this->toNodeId($screen->key);
                $lines[] = "    style {$nodeId} {$colors[$section]}";
            }
        }

        // Стиль для битых ссылок
        foreach ($this->brokenLinks as $link) {
            $brokenId = $this->toNodeId($link['to']) . '_broken';
            $lines[] = "    style {$brokenId} fill:#fecaca,stroke:#dc2626";
        }
